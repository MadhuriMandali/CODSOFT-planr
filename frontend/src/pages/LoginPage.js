  ...
  return (
    <div style={{minHeight:'100vh',background:T.bg,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:18,padding:36,width:'100%',maxWidth:400,boxShadow:'0 32px 64px rgba(0,0,0,.5)'}}>
        ...
        <h1 style={{margin:'0 0 6px',fontSize:22,fontWeight:800,color:T.text}}>Welcome back</h1>
        <p style={{margin:'0 0 28px',fontSize:13,color:T.textMuted}}>Sign in to your workspace</p>
        <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:16}}>
          ...
          <TextInput label="PASSWORD" type="password" value={form.password} onChange={e=>set('password',e.target.value)} placeholder="••••••••"/>
          ...
          <Btn type="submit" full disabled={loading||!form.email||!form.password}>{loading?'Signing in…':'Sign in'}</Btn>
        </form>
        <p style={{marginTop:24,textAlign:'center',fontSize:13,color:T.textMuted}}>
          No account? <Link to="/register" style={{color:T.blue,fontWeight:600,textDecoration:'none'}}>Create one</Link>
        </p>
      </div>
    </div>
  );
